...
def generate_python_script(task):
    algorithms = task.model_algorithms.split(',')
    parameters = json.loads(task.parameters)
    evaluation_methods = task.evaluation_methods.split(',')
    dataset = Database.objects.get(pk=task.dataset_id)
    dataset_path = dataset.get_url()
    ...
    # 构建导入语句
    imports = "\n".join({model_imports[alg] for alg in algorithms if alg in model_imports})

    model_inits = {
        'LinearRegression': 'LinearRegression(fit_intercept={}, normalize={})\n'.format(
            parameters.get('LinearRegression', {}).get('fit_intercept', True),
            parameters.get('LinearRegression', {}).get('normalize', False)
        ),
        'Ridge': 'Ridge(alpha={}, solver="{}")\n'.format(
            parameters.get('Ridge', {}).get('alpha', 1.0),
            parameters.get('Ridge', {}).get('solver', 'auto')
        ),
        'Lasso': 'Lasso(alpha={}, max_iter={})\n'.format(
            parameters.get('Lasso', {}).get('alpha', 1.0),
            parameters.get('Lasso', {}).get('max_iter', 1000)
        ),
        'ElasticNet': 'ElasticNet(alpha={}, l1_ratio={})\n'.format(
            parameters.get('ElasticNet', {}).get('alpha', 1.0),
            parameters.get('ElasticNet', {}).get('l1_ratio', 0.5)
        ),
        'DecisionTreeRegressor': 'DecisionTreeRegressor(max_depth={}, min_samples_split={})\n'.format(
            parameters.get('DecisionTreeRegressor', {}).get('max_depth', None),
            parameters.get('DecisionTreeRegressor', {}).get('min_samples_split', 2)
        ),
        'LogisticRegression': 'LogisticRegression(penalty="{}", C={})\n'.format(
            parameters.get('LogisticRegression', {}).get('penalty', 'l2'),
            parameters.get('LogisticRegression', {}).get('C', 1.0)
        ),
        'KNeighborsClassifier': 'KNeighborsClassifier(n_neighbors={}, weights="{}")\n'.format(
            parameters.get('KNeighborsClassifier', {}).get('n_neighbors', 5),
            parameters.get('KNeighborsClassifier', {}).get('weights', 'uniform')
        ),
        'SVC': 'SVC(C={}, kernel="{}", probability={})\n'.format(
            parameters.get('SVC', {}).get('C', 1.0),
            parameters.get('SVC', {}).get('kernel', 'rbf'),
            True  # probability
        ),
        'DecisionTreeClassifier': 'DecisionTreeClassifier(max_depth={}, min_samples_split={})\n'.format(
            parameters.get('DecisionTreeClassifier', {}).get('max_depth', None),
            parameters.get('DecisionTreeClassifier', {}).get('min_samples_split', 2)
        ),
        'RandomForestClassifier': 'RandomForestClassifier(n_estimators={}, max_depth={})\n'.format(
            parameters.get('RandomForestClassifier', {}).get('n_estimators', 100),
            parameters.get('RandomForestClassifier', {}).get('max_depth', None)
        )
    }

    # 构建模型初始化代码
    # models_code = "\n".join([f"model_{alg} = {model_inits[alg]}" for alg in algorithms if alg in model_inits])

    # models_fit_code = ""
    # for alg in algorithms:
    #     models_fit_code += f"model_{alg}.fit(X_train, y_train)\n"
    models_fit_code = "\n".join([f"model_{alg} = model_pipelines['{alg}'].fit(X_train, y_train)" for alg in algorithms])

    evaluation_code = generate_evaluation_code(task, algorithms, evaluation_methods)
    # df = pd.read_csv('{BASE_DIR}{dataset_path}')
    script_content = f"""
{imports}
...
# 加载数据集
try:
    df = pd.read_csv('{settings.MEDIA_ROOT}\\databases\\{dataset.name}')
except:
...
# 确定数值和分类特征
numeric_features = df.drop('{task.target_column}', axis=1).select_dtypes(include=['int64', 'float64']).columns
categorical_features = df.drop('{task.target_column}', axis=1).select_dtypes(include=['object']).columns
...
{evaluation_code}
"""
    script_filename = f"{task.id}_{task.name}.py"
    return script_content, script_filename


def generate_evaluation_code(task, algorithms, evaluation_methods):
    evaluation_code = ""
    for alg in algorithms:
        for method in evaluation_methods:
            if method == 'Mean Squared Error':
                evaluation_code += f"print('MSE for {alg}:', mean_squared_error(y_test, model_{alg}.predict(X_test)))\n\n"
            elif method == 'R-squared':
                evaluation_code += f"print('R2 for {alg}:', r2_score(y_test, model_{alg}.predict(X_test)))\n\n"
            elif method == 'Accuracy':
                evaluation_code += f"print('Accuracy for {alg}:', accuracy_score(y_test, model_{alg}.predict(X_test)))\n\n"
            elif method == 'Precision-Recall':
                evaluation_code += f"precision, recall, _ = precision_recall_curve(y_test, model_{alg}.predict_proba(X_test)[:, 1])\n" \
                                   f"plt.figure()\n" \
                                   f"plt.plot(recall, precision, marker='.')\n" \
                                   f"plt.title('Precision-Recall for {alg}')\n" \
                                   f"plt.xlabel('Recall')\n" \
                                   f"plt.ylabel('Precision')\n" \
                                   f"os.makedirs(f'{settings.MEDIA_ROOT}/{task.name}/outputs',exist_ok=True)\n" \
                                   f"plt.savefig(f'{settings.MEDIA_ROOT}/{task.name}/outputs/Precision-Recall for {alg}.png')\n" \
                                   f"plt.close()\n\n"
            elif method == 'ROC Curve':
                evaluation_code += f"fpr, tpr, _ = roc_curve(y_test, model_{alg}.predict_proba(X_test)[:, 1])\n" \
                                   f"roc_auc = auc(fpr, tpr)\n" \
                                   f"RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=roc_auc, estimator_name='{alg}').plot()\n" \
                                   f"plt.title('ROC Curve for {alg}')\n" \
                                   f"os.makedirs(f'{settings.MEDIA_ROOT}/{task.name}/outputs',exist_ok=True)\n" \
                                   f"plt.savefig(f'{settings.MEDIA_ROOT}/{task.name}/outputs/ROC Curve for {alg}.png')\n" \
                                   f"plt.close()\n\n"
